...
export default function RecruitPage() {
  ...
  if (status === 'success') return (
    <div style={{ maxWidth: 560, margin: '6rem auto', padding: '0 1rem', textAlign: 'center' }}>
      <div style={{ fontSize: '3rem', marginBottom: '1rem' }} role="img" aria-label="Trophy"><Trophy className="w-4 h-4 inline" /></div>
    ...
    <div style={{ maxWidth: 680, margin: '3rem auto', padding: '0 1rem 5rem' }}>
      ...
      <form onSubmit={handleSubmit} style={{ background: 'var(--psp-card-bg, #fff)', border: '1px solid var(--psp-gray-200, #e5e7eb)', borderRadius: 12, padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.4rem' }}>
        ...
        <button type="submit" disabled={status === 'submitting'}
          ...
          {status === 'submitting' ? 'Submitting\u2026' : <><Trophy className="w-5 h-5 inline" /> Submit Recruiting Profile</>}
        </button>
      </form>
    </div>
  );
