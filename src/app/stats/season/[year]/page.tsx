...
async function getSeasonLeaders(year: string) {
  ...
  for (const cat of fbCols) {
    try {
      const { data } = await supabase.from('football_player_seasons')
        .select(`${cat.col}, players!inner(slug, name, schools!primary_school_id(name))`)
        .eq('season_id', season.id).gt(cat.col, 0).order(cat.col, { ascending: false }).limit(5);
  ...
  for (const cat of bkCols) {
    try {
      const { data } = await supabase.from('basketball_player_seasons')
        .select(`${cat.col}, players!inner(slug, name, schools!primary_school_id(name))`)
        .eq('season_id', season.id).gt(cat.col, 0).order(cat.col, { ascending: false }).limit(5);
