...
interface PotwClientProps {
  initialNominees: PotwNominee[];
  initialWinners: PotwWinner[];
}

export default function PotwClient({ initialNominees, initialWinners }: PotwClientProps) {
  const router = useRouter();
  const [nominees, setNominees] = useState<PotwNominee[]>(initialNominees);
  const [winners, setWinners] = useState<PotwWinner[]>(initialWinners);
  ...
  }, [initialNominees, initialWinners]);
  ...
  async function fetchData() {
    try {
      ...
      setNominees((nomineeRes.data as PotwNominee[] | null) || []);
      setWinners((winnerRes.data as PotwWinner[] | null) || []);
      router.refresh();
    } catch (error) {
