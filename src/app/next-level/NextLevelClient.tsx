...
interface NextLevelClientProps {
  athletes: ProAthlete[];
  stats: { nfl: number; nba: number; mlb: number; wnba: number; total: number };
}

function ProAthleteGrid({ athletes }: { athletes: ProAthlete[] }) {
...
export default function NextLevelClient({ athletes, stats }: NextLevelClientProps) {
  const [selectedLeague, setSelectedLeague] = useState('all');
  const [searchTerm, setSearchTerm] = useState('');

  // Filter athletes client-side
  const filteredAthletes = athletes.filter((a) => {
    const matchesLeague = selectedLeague === 'all' || a.pro_league === selectedLeague;
  ...
  const nflCount = athletes.filter((a) => a.pro_league === 'NFL').length;
  const nbaCount = athletes.filter((a) => a.pro_league === 'NBA').length;
  const mlbCount = athletes.filter((a) => a.pro_league === 'MLB').length;
  const wnbaCount = athletes.filter((a) => a.pro_league === 'WNBA').length;

  // Top producer schools from full dataset
  ...
  return (
    <div className="espn-container" style={{ flex: 1 }}>
      <main>
        ...
        {/* Athlete Cards Grid */}
        <ProAthleteGrid athletes={filteredAthletes} />
      </main>
        ...
        <div className="widget">
          ...
          <div className="w-body">
            <div className="w-row">
              ...
              <span className="val">{stats.total || athletes.length}</span>
            </div>
            <div className="w-row">
